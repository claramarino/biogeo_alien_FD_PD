...
fd_mod <- psem(pop, GDP, connect, static, modif, 
                 ...
                 data = all_ok)
d1 <- dSep(fd_mod)
fisherC(fd_mod) # F.C = 41.49, df = 36, P = 0.243 
d1[which.max(abs(d1$Crit.Value)),] 
# adding correlated error between native diversities and human factors
# until the pval is above .05 => ok here
...
alien_SES_PD <- lmer(SES_PD_alien ~ Area + Dist + Lat + Elev + SLMP + SR_nat + SES_PD_nat +
                       pop + GDP + modif_change + static_modif + connect + SR_alien + 
                       (1|Realm), data = all_ok)
alien_SES_FD <- lmer(SES_FD_alien ~ Area + Dist + Lat + Elev + SLMP + SR_nat + SES_FD_nat +
                         pop + GDP + modif_change + static_modif + connect + SR_alien +
                         (1|Realm), data = all_ok)
...
SES_fd_mod <- psem(pop, GDP, connect, static, modif, 
                     nat_SR, nat_SES_FD, alien_SR, alien_SES_FD,
                     SR_nat %~~% connect,
                     SES_FD_nat %~~% pop,
                     data = all_ok)
d1 <- dSep(SES_fd_mod)
...
# fd
saveRDS(fd_mod, "Output/10_SEM_FD_realm_model.rds")
...
# SES_fd
saveRDS(SES_fd_mod, "Output/10_SEM_sesFD_realm_model.rds")
